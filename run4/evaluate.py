...
if __name__ == "__main__":
    ...
    # get each number's ratio in the label column
    ...
    # init GAN model
    print("initializing generation procedure ...")
    ...
    if opt.downstream_task == "classification":
        ...
        for i in range(0, len(learners)):
            classifier = learners[i].fit(X_syn, y_syn)
            pred_probs = classifier.predict(X_test)
            auc_score = classifier.score(y_test, pred_probs)
            print('-' * 40)
